...
public class buyPoint : MonoBehaviour
{
    ...
    private void OnTriggerStay(Collider other)
    {
        ...
        if(cost == 0)
        {
            ...
            spawner.transform.position = transform.position - new Vector3(0, 4f, 0);
            spawner.tag = "playerSpawner";
            if (spawner.transform.position.z > 0)
                spawner.transform.GetChild(1).position = spawner.transform.position + new Vector3(3, 0, 0);
            else if (spawner.transform.position.z <= 0)
                spawner.transform.GetChild(1).position = spawner.transform.position - new Vector3(3, 0, 0);
            
            spawner.transform.GetChild(1).rotation = Quaternion.Euler(90, 0, 0);
            spawner.transform.DOMoveY(1.6f, 1f).SetEase(Ease.Linear);
            Destroy(gameObject);
        }
